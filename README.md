...
An implementation plan for a deterministic, JSON-driven synthetic data platform built with C# and .NET. The platform is designed to generate correlated users and activity events, validate relationships before execution, publish events through Kafka, persist durable data in PostgreSQL, coordinate jobs with Redis, and export CSV, Parquet, and ZIP artifacts.

This repository is currently in the design and engineering-baseline phase. The specifications describe the target system and the phased path toward a working vertical slice.
